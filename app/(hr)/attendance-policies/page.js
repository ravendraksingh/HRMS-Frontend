...
const AttendancePoliciesPage = () => {
  ...
  const handleSaveNew = async () => {
    ...
    try {
      ...
      const errorMsg = error?.response?.data?.message || error?.response?.data?.error || "Failed to add policy";
      setError(errorMsg);
  ...
  const handleSaveEdit = async (policy) => {
    try {
      ...
      const errorMsg = error?.response?.data?.message || error?.response?.data?.error || "Failed to update policy";
      setError(errorMsg);
      toast.error(errorMsg);
    }
  };
