...
const EmployeesPage = () => {
  ...
  useEffect(() => {
    async function fetchFilters() {
      try {
        ...
        const mgrData = mgrRes.data?.managers || mgrRes.data || [];
        setManagers(mgrData);
      } catch (err) {
        console.log(err);
      }
  ...
  useEffect(() => {
    if (!selectedDept) {
      ...
      setFilteredManagers(
        managers.filter((m) => (m.department_id || m.department) === selectedDept)
      );
    }
    ...
    if (
      selectedManager &&
      !managers.some(
        (m) => (m.department_id || m.department) === selectedDept && (m.empid || m.emp_id) === selectedManager
      )
    ) {
      setSelectedManager("");
    }
        ...
        <Select onValueChange={handleDeptChange} value={selectedDept}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {departments?.map((d) => (
                <SelectItem value={d.deptid || d.id} key={d.deptid || d.id}>
                  {d.name}
                </SelectItem>
              ))}
            </SelectGroup>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {filteredManagers.map((m, index) => (
                <SelectItem value={m.empid} key={m.empid}>
                  {m.name || m.employee_name || m.empid}
                </SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
